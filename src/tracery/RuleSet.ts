...
import { fyShuffle, range } from './Util';
	...
	selectRule(errors: ErrorLog = []): RawRule | null {
		...
		if (this.conditionalRule) {
			...
			if (this.conditionalValues[value.toString()]) {
				const v = this.conditionalValues[value.toString()].selectRule(
					errors
				);
		...
		if (this.defaultRules !== undefined) {
			...
			switch (distribution) {
				...
				default:
					index = Math.floor(
						Math.pow(this.tracery.random(), this.falloff) *
			...
			if (this.defaultUses === null) this.defaultUses = [];
